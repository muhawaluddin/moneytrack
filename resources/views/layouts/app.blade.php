...
const menu=document.querySelector('#mobile-menu'),menuTrigger=document.querySelector('.more-menu-trigger'),menuClose=menu?.querySelector('[data-menu-close]');function setMenu(open){if(!menu||!menuTrigger)return;document.body.classList.toggle('menu-open',open);menu.setAttribute('aria-hidden',String(!open));menuTrigger.setAttribute('aria-expanded',String(open));if(open){setTimeout(()=>menuClose?.focus(),220)}else if(menu.contains(document.activeElement)){menuTrigger.focus()}}menuTrigger?.addEventListener('click',event=>{event.preventDefault();setMenu(true)});document.querySelectorAll('[data-menu-close]').forEach(el=>el.addEventListener('click',()=>setMenu(false)));menu?.querySelectorAll('a').forEach(link=>link.addEventListener('click',()=>setMenu(false)));document.addEventListener('keydown',event=>{if(event.key==='Escape'&&document.body.classList.contains('menu-open'))setMenu(false);if(event.key==='Tab'&&document.body.classList.contains('menu-open')){const focusable=[...menu.querySelectorAll('a,button:not([disabled])')];if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}}});
