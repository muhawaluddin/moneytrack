...
<head>
    ...
    <link rel="stylesheet" href="/app.css"><link rel="stylesheet" href="/sharing.css"><link rel="stylesheet" href="/dashboard.css"><link rel="stylesheet" href="/interface-polish.css">
...
<nav class="bottom-nav" aria-label="Navigasi utama">
    <a href="{{ route('dashboard') }}" @class(['active'=>request()->routeIs('dashboard')])><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="m3 11 9-8 9 8"/><path d="M5 10v10h14V10M9 20v-6h6v6"/></svg><span>Beranda</span></a>
    <a href="{{ route('transactions.index') }}" @class(['active'=>request()->routeIs('transactions.*')])><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M7 4v16m0-16-3 3m3-3 3 3M17 20V4m0 16 3-3m-3 3-3-3"/></svg><span>Transaksi</span></a>
    <a href="{{ route('budgets.index') }}" @class(['active'=>request()->routeIs('budgets.*')])><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v10M15 9.5c0-1.2-1.3-2-3-2s-3 .8-3 2 1.3 2 3 2 3 .8 3 2-1.3 2-3 2-3-.8-3-2"/></svg><span>Anggaran</span></a>
    <a href="{{ route('reports.index') }}" @class(['active'=>request()->routeIs('reports.*')])><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V10M10 20V4M16 20v-7M22 20H2"/></svg><span>Laporan</span></a>
    <button @class(['more-menu-trigger','active'=>request()->routeIs('notifications.*','goals.*','closings.*','accounts.*','spaces.*','categories.*','settings.*')]) type="button" aria-expanded="false" aria-controls="mobile-menu"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><circle cx="5" cy="12" r="1" fill="currentColor"/><circle cx="12" cy="12" r="1" fill="currentColor"/><circle cx="19" cy="12" r="1" fill="currentColor"/></svg><span>Menu</span></button>
</nav>
...
<script>
const theme=document.documentElement.dataset.theme;if(theme==='dark'||(theme==='system'&&matchMedia('(prefers-color-scheme:dark)').matches))document.body.classList.add('dark');if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js');
const menu=document.querySelector('#mobile-menu'),menuTrigger=document.querySelector('.more-menu-trigger'),menuClose=menu?.querySelector('[data-menu-close]');function setMenu(open){if(!menu||!menuTrigger)return;document.body.classList.toggle('menu-open',open);menu.setAttribute('aria-hidden',String(!open));menuTrigger.setAttribute('aria-expanded',String(open));if(open){setTimeout(()=>menuClose?.focus(),220)}else if(menu.contains(document.activeElement)){menuTrigger.focus()}}menuTrigger?.addEventListener('click',event=>{event.preventDefault();setMenu(!document.body.classList.contains('menu-open'))});document.querySelectorAll('[data-menu-close]').forEach(el=>el.addEventListener('click',()=>setMenu(false)));menu?.querySelectorAll('a').forEach(link=>link.addEventListener('click',()=>setMenu(false)));document.addEventListener('keydown',event=>{if(event.key==='Escape'&&document.body.classList.contains('menu-open'))setMenu(false);if(event.key==='Tab'&&document.body.classList.contains('menu-open')){const focusable=[...menu.querySelectorAll('a,button:not([disabled])')];if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}}});
